...
// `dit remote add <scheme>://...` fails with "no remote provider found". Every
...
func TestParseURLResolvesAllProviderSchemes(t *testing.T) {
	cases := []struct {
		...
		{"nop", "nop", "nop"},
		{"dit", "https://example.com/org/repo", "dit"},
	}
...
// can't disable host-key checking on an ssh remote and `dit remote add` fails.
