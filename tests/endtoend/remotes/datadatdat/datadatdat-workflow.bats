...
@test "download API: version details endpoint returns v1.5.0" {
  run curl -sf -H "Cookie: datadatdat_token=${DATADATDAT_API_KEY}" "http://127.0.0.1:3000/api/downloads/v1.5.0"
  assert_success
  assert_output --partial '"version":"v1.5.0"'
}
...
@test "download API: binary download returns file for linux-amd64" {
  ...
  run bash -c "curl -sf -H 'Cookie: datadatdat_token=${DATADATDAT_API_KEY}' 'http://127.0.0.1:3000/api/downloads/v1.5.0/linux-amd64' | head -c 1024 | wc -c"
  assert_success
...
@test "download API: binary download has correct content-type header" {
  run curl -sI -H "Cookie: datadatdat_token=${DATADATDAT_API_KEY}" "http://127.0.0.1:3000/api/downloads/v1.5.0/linux-amd64"
  assert_success
