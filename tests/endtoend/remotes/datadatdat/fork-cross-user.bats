...
# Related: https://github.com/datadatdat/datadatdat-remote-server/issues/560
#
...
setup_file() {
  ...
  if is_dev; then
    ...
    docker exec datadatdat-minio mc alias set myminio http://localhost:9000 minioadmin minioadmin 2>/dev/null || true
  fi
}
...
@test "xfork: plant orphaned journal entry in target namespace" {
  ...
  FIRST_ENTRY=$(docker exec datadatdat-minio mc ls myminio/datadatdat-dev/journals/${OWNER_NS}/${SOURCE_REPO}/ 2>/dev/null | head -1 | awk '{print $NF}')
  [ -n "$FIRST_ENTRY" ] || {
    # Set up minio alias if not already configured
    docker exec datadatdat-minio mc alias set myminio http://localhost:9000 minioadmin minioadmin 2>/dev/null
    FIRST_ENTRY=$(docker exec datadatdat-minio mc ls myminio/datadatdat-dev/journals/${OWNER_NS}/${SOURCE_REPO}/ | head -1 | awk '{print $NF}')
  }
  [ -n "$FIRST_ENTRY" ]

  run docker exec datadatdat-minio mc cp \
    "myminio/datadatdat-dev/journals/${OWNER_NS}/${SOURCE_REPO}/${FIRST_ENTRY}" \
    "myminio/datadatdat-dev/journals/${FORKER_NS}/${SOURCE_REPO}/${FIRST_ENTRY}"
  assert_success
}
...
@test "xfork: clean up orphaned data for next test" {
  is_dev || skip "MinIO only available in DEV"
  run docker exec datadatdat-minio mc rm --recursive --force \
    "myminio/datadatdat-dev/journals/${FORKER_NS}/${SOURCE_REPO}/"
  assert_success
}
