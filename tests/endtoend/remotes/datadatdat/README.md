# Datadatdat Remote Server E2E Tests

This directory contains end-to-end tests for the Datadatdat Remote Server.

## Test Files

### datadatdatWorkflowTests.yml
Tests the core datadatdat workflow functionality:
...
- Audit logging
- Rate limiting
- Integration with datadatdat CLI

## Prerequisites

Before running the tests, ensure:

1. **datadatdat-remote-server is running**
   ```bash
   cd datadatdat-remote-server/deploy/compose
   docker-compose up -d
   ```
...
### Run all datadatdat workflow tests:
```bash
...
### Integration Tests
- ✅ Datadatdat CLI with authenticated API
...
docker exec datadatdat-postgres pg_isready  # Database
```

### View service logs:
```bash
docker logs datadatdat-auth-server
docker logs datadatdat-api-gateway
docker logs datadatdat-postgres
```

### Check database state:
```bash
docker exec -it datadatdat-postgres psql -U datadatdat -d datadatdat
```

### Verify JWT configuration:
```bash
docker exec datadatdat-auth-server env | grep JWT
```
...
jobs:
  e2e-auth-tests:
    ...
    steps:
      ...
      - name: Start datadatdat-remote-server
        run: |
          cd datadatdat-remote-server/deploy/compose
          docker-compose up -d
...
|---------|------|----------------|--------------|
| Auth Server | 8085 | datadatdat-auth-server | http://localhost:8085/health |
| API Gateway | 8080 | datadatdat-api-gateway | http://localhost:8080/health |
| PostgreSQL | 5433 | datadatdat-postgres | pg_isready |
| MinIO | 9000 | datadatdat-minio | http://localhost:9000/minio/health/live |
...
- Check the [DEPLOYMENT_GUIDE.md](../../../../datadatdat-remote-server/DEPLOYMENT_GUIDE.md)
- Review [AUTH_IMPLEMENTATION.md](../../../../datadatdat-remote-server/AUTH_IMPLEMENTATION.md)
