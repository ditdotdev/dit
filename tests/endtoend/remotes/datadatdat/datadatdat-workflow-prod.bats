...
# Setup: Verify datadatdat ECS production is running
setup_file() {
  ...
  assert_success
  [[ "$output" == "OK" ]] || {
    echo "datadatdat ECS production is not running or not healthy"
...
teardown_file() {
  ...
  curl -X DELETE -sf -H "Authorization: Bearer ${DATADATDAT_API_KEY}" \
    "https://datadatdat.com/api/v1/repos/e2e-testorg/datadatdat-test" 2>/dev/null || true
  curl -X DELETE -sf -H "Authorization: Bearer ${DATADATDAT_API_KEY}" \
    "https://datadatdat.com/api/v1/repos/e2e-webtest/ui-repo" 2>/dev/null || true
...
@test "create test repository in datadatdat ECS production" {
  run curl -X POST -f -H "Authorization: Bearer ${DATADATDAT_API_KEY}" \
    "https://datadatdat.com/api/v1/repos/e2e-testorg/datadatdat-test"
  assert_success
...
@test "add datadatdat remote succeeds" {
  run "$D3" remote add https://datadatdat.com/e2e-testorg/datadatdat-test datadatdattest
  assert_success
}
...
@test "repo has datadatdat remote" {
  ...
  assert_success
  assert_output --partial "https://datadatdat.com/e2e-testorg/datadatdat-test"
}
...
@test "test list repos by org API endpoint - e2e-testorg (before cleanup)" {
  run curl -sf -H "Authorization: Bearer ${DATADATDAT_API_KEY}" \
    "https://datadatdat.com/api/v1/repos/e2e-testorg"
  assert_success
  assert_output --partial "e2e-testorg/datadatdat-test"
}

@test "cleanup - delete test repository from datadatdat ECS production" {
...
@test "web UI: create test repo for web UI tests" {
  run curl -X POST -f -H "Authorization: Bearer ${DATADATDAT_API_KEY}" \
    "https://datadatdat.com/api/v1/repos/e2e-webtest/ui-repo"
  assert_success
...
@test "web UI: add remote" {
  run "$D3" remote add https://datadatdat.com/e2e-webtest/ui-repo webuitest
  assert_success
}
...
@test "web UI: test commits list API returns initial commit" {
  ...
  run curl -sf -H "Authorization: Bearer ${DATADATDAT_API_KEY}" \
    "https://datadatdat.com/api/v1/repos/e2e-webtest/ui-repo/commits"
  assert_success
...
@test "web UI: test individual commit API" {
  WEB_COMMIT_1=$(cat "$BATS_TMPDIR/web_commit_1.txt")
  run curl -sf -H "Authorization: Bearer ${DATADATDAT_API_KEY}" \
    "https://datadatdat.com/api/v1/repos/e2e-webtest/ui-repo/commits/${WEB_COMMIT_1}"
  assert_success
  assert_output --partial "Initial web UI test commit"
}
...
@test "web UI: verify second commit details via API" {
  WEB_COMMIT_2=$(cat "$BATS_TMPDIR/web_commit_2.txt")
  run curl -sf -H "Authorization: Bearer ${DATADATDAT_API_KEY}" \
    "https://datadatdat.com/api/v1/repos/e2e-webtest/ui-repo/commits/${WEB_COMMIT_2}"
  assert_success
  assert_output --partial "${WEB_COMMIT_2}"
  assert_output --partial "Added test table"
}
...
@test "web UI: test manifest updates on push - verify all 3 commits in manifest" {
  ...
  run curl -s -H "Authorization: Bearer ${DATADATDAT_API_KEY}" \
    "https://datadatdat.com/api/v1/repos/e2e-webtest/ui-repo/manifest"
  assert_success
  assert_output --partial "${WEB_COMMIT_1}"
  assert_output --partial "${WEB_COMMIT_2}"
...
@test "web UI: test clone with manifest - clone from remote" {
  run "$D3" clone -n webuitestclone https://datadatdat.com/e2e-webtest/ui-repo
  assert_success
  assert_output --partial "checked out"
}
