...
are always full sends, as d3 is sending data to a newly created directory.
...
.. note::
   ...
   The SSH connection is made from within the Datadatdat server container, so the host
   ...
   the host system but not within datadatdat, you can run
   ``docker exec -it d3`` and run standard Linux commands
