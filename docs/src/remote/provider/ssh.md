...
are always full sends, as dit is sending data to a newly created directory.
...
> **Note:**
> The SSH connection is made from within the Dit server container, so the host
...
> the host system but not within dit, you can run
> `docker exec -it dit` and run standard Linux commands
