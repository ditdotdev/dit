...
others. Much like git, Datadatdat has the notion of `remote repositories` that
act as an endpoint for push and pull.

There are a few important general things to be aware of:

* Datadatdat commits do not have a strict dependency on the previous commit from
  ...
  by the user.
* Datadatdat does not support the notion of merging. While concepts like tagging
...
* Different remote providers have different performance characteristics,
  ...
  only changed blocks. Datadatdat is designed to work with small
  datasets (<10GB), using it for anything remotely large may have adverse
  effects on the system.

.. warning::

   Datadatdat currently ships with two very basic providers, the :ref:`remote_provider_s3`
   ...
   operated at scale in an enterprise setting. As Datadatdat matures, we will be
