---
title: Dit with Docker
nav_label: Docker
nav_order: 140
---

Dit for docker is designed to run on any system that supports docker, but
...
architecture of Dit. To make dit possible, there is a container
(`dit`) running in the background that provides data
...
out-of-tree kernel modules work, this needs to be done by the dit software (a
container named `dit` in particular). Dit attempts to
...
not on a supported operating system, you may find `dit install` taking a
...
the `2.2.x` or `2.3.x` series, and Dit will use that instead of trying to
install its own.
...
dit ships and tests against three Docker environments. Older paths (Docker
Desktop on macOS with the HyperKit/LinuxKit VM; "Docker for WSL" tech
preview; Docker via boot2docker) are no longer supported.

### macOS — Colima

On macOS, dit runs against [Colima](https://github.com/abiosoft/colima), which
provisions a lightweight Linux VM with a real kernel that dit can load ZFS
...
Install Colima with Homebrew and start it before running `dit install`:
...
On a Linux host, dit runs against the host's native Docker daemon. The host
kernel needs ZFS modules; dit will install or build them for the host's
kernel during `dit install`. The community tries to provide pre-built
binaries for:
...
[zfs-releases](https://github.com/ditdotdev/zfs-releases) repository with
the latest build information. For an unsupported distribution or a new
major version, the
[zfs-builder](https://github.com/ditdotdev/zfs-builder) repository describes
how to add a new build target.

### Windows — Docker Desktop with the WSL2 backend

On Windows, dit runs against Docker Desktop using the WSL2 backend. dit
...
> been published yet, `dit install` will try to build them on the fly inside
> the `zfs-builder` container. If that fails, head over to the
> [zfs-releases](https://github.com/ditdotdev/zfs-releases) repository
...
> [install ZFS](https://github.com/openzfs/zfs) yourself. Dit will
