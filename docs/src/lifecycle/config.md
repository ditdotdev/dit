...
Whenever a new repository is created, Datadatdat will store metadata about the
...
Whenever Datadatdat creates a new container from a commit, Datadatdat will attempt to use
...
if the `postgres:11` image is specified, Datadatdat will prefer the digest of that
image, as new images (corresponding to Postgres minor releases) may be pushed
under that tag. But in the event that digest can't be found, Datadatdat will
...
> **Attention:**
> Datadatdat does not currently perform rigorous compatibility checks when pushing
...
By default, Datadatdat will forward any exposed ports on the container to the local
