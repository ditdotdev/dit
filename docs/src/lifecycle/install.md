...
[Download Page](https://datadatdat.com/download) and download the archive
...
`-r <registry>` option to `d3 install` to pull images from a private
registry instead.

When using the local docker context, the `d3` container is responsible for
installing ZFS on the Docker or host VM. For more information on how this
works and supported configurations, see [Datadatdat with Docker](docker.md).
