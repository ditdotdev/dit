...
Datadatdat relies heavily on docker to provide database capabilities. Datadatdat will
...
If you're unsure if a docker image can be used under datadatdat, look for one or
more volume declarations.

.. note::

   Datadatdat doesn't currently support running images that have been built but
   ...
   a private registry, before using them in datadatdat. This will be fixed in a
   future release.

.. note::

   Datadatdat requires that all volumes be declared within the image metadata.
   ...
   `community <https://datadatdat.com/community>` and file a feature request
   for us to better understand the need.

Docker Containers
-----------------
Every d3 repository corresponds to exactly one docker container. The
...
.. note::
   ...
   All Datadatdat repositories must be run with the ``-d`` (daemonize) flag
   ...
   over the `community <https://datadatdat.com/community>`_ to submit a
   feature request.

The docker container must adhere to some general container principles to
be used by datadatdat:
...
Docker containers can be managed by ``d3, such as
starting and stopping the container. If you ``docker rm`` the container,
it will show up as ``d3, though it can be restarted by
running ``d3 start`` even in the detached state.
...
even if you're using a tag like ``postgres:latest``, d3 will record the
...
.. note::
   ...
   Datadatdat does not currently support changing the docker configuration of a
...
--------------------------
Datadatdat began its life focused on the tool most commonly used by individual
...
container platform such as AWS Fargate. We believe that Datadatdat can and will
...
Keep up to date with our `future roadmap <https://datadatdat.com/future>`_, and
join the `community <https://datadatdat.com/community>`_ to help us evolve
in the future.
