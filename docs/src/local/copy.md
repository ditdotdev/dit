...
Every database has some type of persistent data beneath it. For Dit, we
...
```bash
dit cp -s /path/to/postgres/data mypostgres
```
...
> such as `dit cp -s /path/to/mongo/db -d /data/db mymongo`. You may
